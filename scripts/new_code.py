    ...
    with pdfplumber.open(pdf_path) as pdf:
        for page in pdf.pages:
            ...
            while i < len(lines) - 1:
                ...
                # HANDLE CHARGES / GST
                # =============================
                if is_charge_line(line1) or is_charge_line(line2):

                    combined_block = " ".join(lines[i : i + 5])

                    nums = re.findall(r"\d+(?:,\d{3})*(?:\.\d+)?", combined_block)
                    ...
                    date_match = DATE_RE.search(combined_block)
                    if not date_match:
                        i += 1
                        continue
                    ...
                    rows.append(
                        {
                            "Date": date,
                            "To Whom": extract_charge_name(combined_block),
                            "Amount": amount,
                            "Balance": balance,
                        }
                    )
