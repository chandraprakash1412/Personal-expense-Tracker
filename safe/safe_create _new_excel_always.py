...
OUTPUT_FILE = OUTPUT_DIR / "expense_tracker.xlsx"
    ...
    with pdfplumber.open(pdf_path) as pdf:
        ...
        for page in pdf.pages:
            ...
            while i < len(lines) - 1:
                ...
                # Skip only CHANDRAPRAKASHBARBXX transfers
...
def load_data():
    ...
    # detect withdrawal vs deposit using balance change
    ...
    return df[["Date", "To Whom", "Withdrawal", "Month", "Year"]]


def write_excel(df):

    monthly = (
        df.groupby(["Year", "Month"])["Withdrawal"]
        .sum()
        .reset_index()
        ...
        chart1.add_series(
            {
                ...
                "categories": "=Monthly_Expenses!B2:B200",
                "values": "=Monthly_Expenses!C2:C200",
            }
        )

        chart1.set_title({"name": "Monthly Expenses"})
        chart1.set_x_axis({"name": "Month"})
        chart1.set_y_axis({"name": "Amount"})

        dashboard.insert_chart("B2", chart1)
        ...
        chart2.add_series(
            {
                ...
                "categories": "=Yearly_Expenses!A2:A50",
                "values": "=Yearly_Expenses!B2:B50",
            }
        )

        chart2.set_title({"name": "Yearly Expenses"})
